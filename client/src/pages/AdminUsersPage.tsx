...
export default function AdminUsersPage() {
  ...
  const canEditRole = (u: AdminUserApi) => {
    ...
    if (!isSuperAdmin && (u.role === "ADMIN" || u.role === "SUPERADMIN")) return false;
    ...
    <>
      <div className="min-h-screen bg-[#FFF5ED] px-4 py-20">
        <div className="mx-auto max-w-4xl bg-white/90 border border-[#ead8c7] shadow rounded-2xl p-6 space-y-6">
          <button onClick={() => navigate(-1)} className="text-sm text-slate-600 underline">
            ← Retour
          </button>
                ...
                {filteredUsers.map((u) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="text-xs text-slate-500">Tél : {u.phone ?? "-"}</p>
                    ...
                    <div className="mt-3 flex items-center justify-between gap-2">
                      <select
                        value={u.role}
                        onChange={(e) => handleChangeRole(u.id, e.target.value as UserRoleApi)}
